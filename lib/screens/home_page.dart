      ...
      body: SafeArea(
        child: BlocBuilder<AppCubit, AppState>(
          builder: (context, state) {
            ...
            final overdueModules =
                state.modulesEx.where((m) => m.isOverdue).length;
            final urgentModules = state.modulesEx
                .where((m) => m.priority == ModulePriority.urgent)
                .length;
                ...
                TweenAnimationBuilder<double>(
                  ...
                  builder: (context, value, child) {
                    return Transform.scale(
                      ...
                      child: Opacity(
                        opacity: value,
                        child: _HeroCard(counter: state.settings.counter),
                      ),
                    );
                  },
                ),
                          ...
                          TextButton(
                            onPressed: () {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text(
                                      'Перейдите во вкладку "Модули" для просмотра'),
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        // Чуть выше плитки = гарантированно без overflow даже на десктопе
        final double tileHeight = columns == 1 ? 132 : 124;
